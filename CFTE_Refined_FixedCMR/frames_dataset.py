...
def sample_symmetric_frame_indices(num_frames, temporal_offset=3):
    ...
    For this Refined FixedCMR ablation, k is fixed by the configuration and temporal
...
class FramesDataset(Dataset):
    ...
    def __init__(self, root_dir, frame_shape=(256, 256, 3), id_sampling=False, is_train=True,
        ...
        if fixed_temporal_offset is not None:
            ...
            if int(min_temporal_offset) != int(max_temporal_offset):
                raise ValueError(
                    "Refined FixedCMR requires a single fixed temporal offset; "
    ...
    def _resolve_training_path(self, idx):
        """Resolve one training video path for a dataset index.

        Returning ``None`` means that the selected identity has no matching
        video file; the caller will resample another training item.
        """
        if self.id_sampling:
