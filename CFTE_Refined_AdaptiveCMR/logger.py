...
class Visualizer:
    ...
    def visualize(self, driving, source, out, source_future=None):
        """Create a single-CFTE-style table for the two-reference Refined AdaptiveCMR model.
        ...
        No text is inserted between the image tiles.  The motion sequence is
        shown as initial dense flow -> initial learned CMR residual Delta M(0)
        -> cumulative accepted adaptive residual sum_{k=1}^{K} Delta M(k)
        -> final dense flow.  CMR1 belongs to the Future branch and CMR2 belongs
        to the Past branch.  Deformed always uses the final adaptive motion field.
        """
        ...
        visuals = {
            ...
            # DISTS-guided cumulative accepted correction:
            # sum_{k=1}^{K} Delta M(k), not the final flow itself and not only
            # the last accepted single-step Delta M(K).
            'adaptive_flow_past': self._flow_or_blank(
                out, 'deformation_delta_past_adaptive',
                batch_size, height, width),
            'adaptive_flow_future': self._flow_or_blank(
                out, 'deformation_delta_future_adaptive',
                batch_size, height, width),
            ...
            # These are the final branch outputs after both learned CMR
            # refinement and DISTS-guided adaptive motion correction.
        ...
        for idx in range(batch_size):
            ...
            for row_label, batches, cmr_annotation, adaptive_annotation in branch_rows:
                tiles = []
                for column_idx, batch in enumerate(batches):
                    tile = self._image_tile(batch, idx)
                    # Sixth image column: learned CMR residual.
                    if column_idx == 5:
                        tile = self._annotate_tile(tile, cmr_annotation)
                    # Seventh image column: DISTS-guided adaptive residual.
                    elif column_idx == 6:
                        tile = self._annotate_tile(tile, adaptive_annotation)
                    tiles.append(tile)
        ...
        labels = [
            ...
            'Initial Dense Flow', 'Initial CMR ΔM(0)',
            'Adaptive ΣΔM(k)', 'Final Dense Flow', 'Deformed',
            'Superposition', 'Occlusion Map', 'Prediction', 'Target'
        ]
        column_widths = [self.row_label_width] + [width] * 13
