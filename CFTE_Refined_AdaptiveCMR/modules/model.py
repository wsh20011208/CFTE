    ...
    def forward(self, x, lambda_var):
        ...
        if hasattr(self.generator_past, 'forward_superposed_after_deform'):
            fused_generated = self.generator_past.forward_superposed_after_deform(
                ...
                target_image=driving, dists_model=self.dists)
        ...
        # Adaptive correction diagnostics.  These errors are DISTS(superposed deformed image, target),
        # not MAE.  The optional adaptive DISTS loss trains the CMR/adaptive motion fields so that
        # the superposed deformed image is closer to the target before final decoding.
        if 'adaptive_dists_initial' in generated and 'adaptive_dists_final' in generated:
            adaptive_initial = generated['adaptive_dists_initial']
            adaptive_final = generated['adaptive_dists_final']
            loss_values['metric_adaptive_dists_initial'] = adaptive_initial.mean().detach()
            loss_values['metric_adaptive_dists_final'] = adaptive_final.mean().detach()
            loss_values['metric_adaptive_dists_improvement'] = (adaptive_initial - adaptive_final).mean().detach()
            loss_values['metric_adaptive_iterations'] = generated['adaptive_iterations'].mean().detach()
            loss_values['metric_adaptive_delta_mean'] = generated['adaptive_delta_mean'].mean().detach()

            # Per-iteration DISTS curve for convergence diagnosis.
            # metric_adaptive_dists_iter_00 is the CMR1/CMR2 initial DISTS;
            # metric_adaptive_dists_iter_01..N are accepted adaptive updates, padded
            # with the final DISTS after early stopping.  Logger averages these over
            # each epoch and saves a curve plot.
            if 'adaptive_dists_curve' in generated:
                adaptive_curve = generated['adaptive_dists_curve']
                if adaptive_curve.dim() == 1:
                    adaptive_curve = adaptive_curve.unsqueeze(0)
                for iter_idx in range(adaptive_curve.shape[1]):
                    loss_values['metric_adaptive_dists_iter_%02d' % iter_idx] = adaptive_curve[:, iter_idx].mean().detach()
            if 'adaptive_dists_curve_length' in generated:
                loss_values['metric_adaptive_dists_curve_length'] = generated['adaptive_dists_curve_length'].mean().detach()

            adaptive_dists_weight = self.loss_weights.get('adaptive_dists', 0.0)
            if adaptive_dists_weight != 0:
                loss_values['loss_adaptive_dists'] = adaptive_dists_weight * adaptive_final.mean()

            adaptive_sequence_weight = self.loss_weights.get('adaptive_dists_sequence', 0.0)
            if adaptive_sequence_weight != 0 and 'adaptive_dists_sequence_mean' in generated:
                loss_values['loss_adaptive_dists_sequence'] = adaptive_sequence_weight * generated['adaptive_dists_sequence_mean'].mean()

            adaptive_improvement_weight = self.loss_weights.get('adaptive_dists_improvement', 0.0)
            if adaptive_improvement_weight != 0:
                # Penalize only cases where the adaptive final DISTS is worse than the CMR initial DISTS.
                loss_values['loss_adaptive_dists_improvement'] = adaptive_improvement_weight * \
                    torch.relu(adaptive_final.mean() - adaptive_initial.detach().mean())

            adaptive_delta_weight = self.loss_weights.get('adaptive_delta', 0.0)
            if adaptive_delta_weight != 0:
                loss_values['loss_adaptive_delta'] = adaptive_delta_weight * generated['adaptive_delta_mean'].mean()
