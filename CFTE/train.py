    ...
    with Logger(log_dir=log_dir, visualizer_params=config['visualizer_params'], checkpoint_freq=train_params['checkpoint_freq']) as logger:
        for epoch in trange(start_epoch, train_params['num_epochs']):
            for x in dataloader:    
                ...
                # lambda, bpp, and dists are logged separately.  The true
                # rate-distortion contribution is included only through rdloss.
                logging_only_keys = {'lambda', 'bpp', 'dists'}
                loss_values = [
                    val.mean()
                    for key, val in losses_generator.items()
                    if key not in logging_only_keys
                ]
