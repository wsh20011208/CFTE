...
With the default gap=3 and stride=3, target frames are exactly:
    3, 6, 9, 12, ...
...
def _target_candidates(num_frames: int, gap: int, stride: int):
    ...
    # For the default stride=3, this gives 3, 6, 9, 12, ...
    candidates = []
    t = stride
    while t < num_frames:
...
def main():
    ...
    parser.add_argument('--output_csv', default='benchmark/test_triplets_gap3_stride3.csv')
    parser.add_argument('--split', default='test', help='Subdirectory to use if it exists under root_dir. Default: test')
    parser.add_argument('--gap', type=int, default=3, help='Reference-frame gap. Default: past=t-3 and future=t+3.')
    ...
    parser.add_argument('--subsample_strategy', choices=['first', 'uniform'], default='first',
                        help='When max_targets_per_video is positive: first keeps 3,6,9,...; uniform spreads N targets over the video.')
