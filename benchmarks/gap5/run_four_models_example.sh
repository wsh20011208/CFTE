...
STRIDE="${STRIDE:-3}"
DEVICE="${DEVICE:-0}"
ROOT_DIR="${ROOT_DIR:-$REPO_ROOT/vox}"
OUT_ROOT="${OUT_ROOT:-$REPO_ROOT/benchmark_results/gap5}"

SINGLE_DIR="${SINGLE_DIR:-$REPO_ROOT/CFTE}"
...
"$PYTHON_BIN" "$SCRIPT_DIR/make_test_triplets_gap5_stride3.py" \
  --root_dir "$ROOT_DIR" \
  --gap "$GAP" \
  --stride "$STRIDE" \
