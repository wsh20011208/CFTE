...
With the default gap=7 and stride=3, candidate target frames remain:
    3, 6, 9, 12, ...

After the symmetric boundary test, the first valid target for gap=7 is 9:
    past=2, target=9, future=16
...
def main():
    ...
    parser.add_argument('--output_csv', default='benchmark/test_triplets_gap7_stride3.csv')
    parser.add_argument('--split', default='test', help='Subdirectory to use if it exists under root_dir. Default: test')
    parser.add_argument('--gap', type=int, default=7, help='Symmetric reference-frame gap. Default: past=t-7 and future=t+7.')
