...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x, lambda_var):
        ...
        loss_values['bpp'] = bpp_mv.detach()

        ### differentiable DISTS distortion over the complete mini-batch
        prediction = generated['prediction'].clamp(0, 1)
        groundtruth = x['driving'].clamp(0, 1)
        dists = self.dists(groundtruth, prediction, as_loss=True) ######
        loss_values['dists'] = dists.detach()

        #### true rate-distortion loss used for optimization
