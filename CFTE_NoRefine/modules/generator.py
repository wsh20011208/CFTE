...
class OcclusionAwareGenerator(nn.Module):
    ...
    def __init__(self, num_channels, num_kp, block_expansion, max_features, num_down_blocks,
        ...
        # Post-superposition occlusion branch for the no-refinement two-reference model.
        # The past/future branch deformed representations are fused first; then a
        # single occlusion map is predicted and used to decode the final image.
    ...
    def forward_superposed_after_deform(self, source_past, source_future, generated_past, generated_future,
        ...
        """
        No-refinement deformation-level superposition path.

        This baseline intentionally does not refine either motion field.  It uses
        the raw past/future deformations produced by the two CFTE branches, warps
        the branch features/images, superposes the deformed representations, then
        predicts one post-superposition occlusion map before final decoding.

        The returned dictionary uses the same diagnostic key names as the
        CMR1/CMR2 model where possible.  For example, *_raw and refined flow keys
        are both set to the same raw deformation, and CMR delta fields are zero.
        This keeps the logger layout directly comparable with FixedCMR.
        """
        deformation_past_raw = generated_past['deformation']
        deformation_future_raw = generated_future['deformation']

        deformed_feature_past = self.deform_input(generated_past['encoded_feature'], deformation_past_raw)
        deformed_feature_future = self.deform_input(generated_future['encoded_feature'], deformation_future_raw)

        deformed_past = self.deform_input(source_past, deformation_past_raw)
        deformed_future = self.deform_input(source_future, deformation_future_raw)
        ...
        fused_deformation = weight_past * deformation_past_raw + weight_future * deformation_future_raw
        ...
        return {
            ...
            'deformation_past': deformation_past_raw,
            'deformation_future': deformation_future_raw,
